...
- Reuse screen patterns from ProcessMonitor, WorkflowBuilder, AIAgentProjects, SharedSpaceFiles, Nodes, and SubjectHub.
- Every generated app needs a dashboard, at least three domain routes, a records route, an operations route, and a settings route.
- Pages must include realistic copy, status labels, table rows, forms, and workflow actions derived from the prompt domain.
